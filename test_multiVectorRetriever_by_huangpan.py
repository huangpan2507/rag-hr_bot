...
class LoadEnsembleRetriever:
    ...
    def create_ensembleRetriever(self, bm25_retriever):
        multiVectorRetriever = self.create_multiVector_retriever()
        ensemble_retriever = EnsembleRetriever(
            retrievers=[bm25_retriever, multiVectorRetriever], weights=[0.5, 0.5]
        )
        return ensemble_retriever

    def fetch_relevant_documents_by_ensembleRetriever(self, query):
        retriever, valid_docs = self.create_bm25_retriever()
        ensemble_retriever = self.create_ensembleRetriever(retriever)
        results = ensemble_retriever.invoke(query)
        return results
    
    def fetch_relevant_documents_by_multiVectorRetriever(self, query):
        multiVector_retriever = self.create_multiVector_retriever()
        results = multiVector_retriever.invoke(query)
        return results
...
relevant_documents = db_loader.fetch_relevant_documents_by_multiVectorRetriever(query)
